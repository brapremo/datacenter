...
def get_serial(module):
    # retreive serial number for module
...
def check_for_errors(module):
    # check if ASIC CRC counters are incrementing
    # error_dict=dict()
    ...
    for line in output:
        ...
        if match and match.group(1) and match.group(2):
            if debug:
                ...
                print('adding value: {0} to key: {1}'.format(str(
                      match.group(1)).strip('0'), match.group(2)))
        ...
        for module in current_counters.keys():
            if module in old_counters:
                if current_counters[module] > old_counters[module]:
                    ...
                    if debug:
                        print(('Counters for module {0} have '
                               'incremented').format(module))
                ...
                # output
                if debug:
                    print('no previous stats for module '
                          '{0}'.format(module))
    else:
